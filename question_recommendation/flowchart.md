...
flowchart TD
    ...
    H --> H2{"特殊卡是否匹配<br/>原问题对象词？"}
    H2 -- 是 --> H3["按兼容特殊卡过滤<br/>不兼容文本设备值"]
    H2 -- 否 --> H4["其他 Basic 候选生成"]
    H3 --> H4

    K --> K2{"特殊卡是否通过<br/>设备和对象硬过滤？"}
    K2 -- 是 --> K3["按三层回退链确定<br/>候选设备类型"]
    K2 -- 否 --> K4["其他候选生成"]
    ...
    Q --> X["生成字段继承策略<br/>和 simplify 可删约束"]
    R --> X
    X --> S["组装动态 Prompt"]
...
| 特殊设备词分析 | 无结构化设备时，从原问题提取已知设备词并按特殊卡兼容性过滤，为候选提供设备方向 |
| 字段边界 | 有实时元数据时以实时元数据为准；无实时元数据时以候选能力的 `properties` 和 `metrics` 为准 |
| 字段继承策略 | 当上下文没有结构化属性或 KPI 时，禁止把原问题中的字段表达直接当作可继承字段 |
...
- 绑定特殊能力候选时，不得继承未出现在候选 `device_types` 中的设备词；原问题设备词未进入候选但 objects 可用时，保留告警、链路等特殊方向，改用通用或候选支持的设备类型方向。
...
当无结构化设备但有原问题时，系统会对特殊能力执行设备词兼容性分析：

- 从问题文本中提取已知设备词（通过 `specific_terms_in_text`）
- 对每个匹配的特殊卡，过滤不兼容的设备词
- 使用三层回退链确定候选设备类型：结构化命中设备 → 文本兼容词 → 卡全量支持类型
- 只有兼容设备词才进入特殊能力的上下文构造
...
- 字段继承策略会标记原问题属性或 KPI 是否可继承；当上游未结构化提供字段时，LLM 只能从绑定候选字段中选择具体属性或指标。
- `simplify` 场景会额外生成可删除约束，只允许 LLM 基于这些结构化约束降低复杂度。
- 动态 Prompt 由核心规则、恢复策略、子网/元数据场景片段、候选能力、字段分析、字段继承策略和简化分析组成。
