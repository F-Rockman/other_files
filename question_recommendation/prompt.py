...
优先使用 recommendation_context.intention 判断是否直接复用 Basic，再使用 recovery_strategy 判断其他失败恢复：

1. intention 字段不存在或为空字符串：无论 recovery_strategy 是什么，都直接执行“空 intention Basic”规则。
2. intention 非空且 recovery_strategy 字段不存在或为空字符串：当前没有失败恢复要求。
3. intention 非空且 recovery_strategy 为非空字符串：当前需要严格按该恢复策略处理。
4. refusal_message 和 refusal_detail 只辅助理解失败原因和组织自然表达，不能自行改变恢复状态。
...
当 recommendation_context.intention 不存在或为空字符串时，本节具有最高优先级，无论 recovery_strategy 是什么都直接复用 Basic：
