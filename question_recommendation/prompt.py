...
10. 推荐问题可以使用列表、数量、字段聚合、趋势和 TopN 等现有查询形式，但不得生成同比、环比、较上期、较同期、去年同期、上月同期等跨周期对比，也不得生成由此派生的增长率、变化率或增减幅问题。即使原始 question 明确包含这类对比表达，推荐中也必须移除，改用候选范围内的趋势、聚合、TopN 或其他可执行方向。
...
- 同类型变换最多生成 1 条推荐。变换类型包括指标替换、属性替换、删除时间、删除定位条件、删除过滤条件、删除聚合、删除对象条件、范围调整和结果形态变化；例如已有一条推荐使用指标替换，其余推荐不得再使用指标替换。不足 3 条时，按当前场景的退化路径补足，而不是继续重复同类变换。若当前场景对某类变换有更严格限制，例如 simplify 禁止指标替换，则以更严格限制为准。
...
9. 不复述 invalid_values。提及的设备和子部件名称必须逐字沿用 recommends 中实际使用的名称，并与 recommendation_context 中的真实有效表达保持一致；禁止用父类或泛化名称替换，例如不得把"设备类型A"改写为其父类"设备类型B"。有子部件时保留父子关系；多个设备类型时不归因于单一类型；没有明确设备类型时不虚构对象。
...
12. 原因部分禁止使用无对象归属的"属性1在现有数据中暂未匹配到相关内容"等表达，必须说明"当前可查询的设备类型A信息中暂未匹配到属性1相关内容"。
13. "设备类型A、设备类型B、设备A、属性1、指标1、IP地址A"等仅为 Prompt 规则示例占位词。最终 recommends 和 explain 必须替换为输入中的真实有效表达，禁止原样输出这些占位词。
14. 使用"暂未匹配到对应对象/相关属性内容/合适业务取值""暂未采集到相关指标数据""暂未识别到可用关联""当前条件下暂未查询到相关数据"等委婉表达。
15. 禁止使用“设备不存在”“字段不存在”“对象没有该属性/指标”“不支持查询该字段/指标”“暂不支持该查询”及内部技术表达。"""
...
必须输出正好 3 条推荐。输出前逐条检查：候选边界、对象关系、无效值、明确结果形态、与原问题差异、三条间差异；explain 是否包含当前提问、当前原因和下一步方向（恢复场景）或当前提问和下一步方向（普通场景）；是否保留真实对象和有效条件，是否仅使用用户可理解的业务表达。
"""
...
简化优先于参数、子网和结果形态继承。保留原业务对象、父子关系及至少一个核心目标；每条推荐必须删除至少一个真实查询条件，禁止保留全部条件或追加新条件。真实查询条件包括时间、子网、定位值、过滤条件、聚合、分组、额外设备条件、额外对象、额外 KPI 或额外属性。

删除“展示趋势”“趋势”“查看趋势”等查询形态词不算有效简化；查指标时无论是否有时间范围，不写“趋势”通常仍等价为趋势查询，不能只靠省略趋势生成推荐。TopN 是查询形态，不算有效查询条件；删除 TopN、Top5、排名最高等表达不算有效简化。不得只把 KPI 做轻微泛化，例如“总带宽”改成“带宽”但保留所有核心条件。
...
- candidate_field_analysis 中的 unsupported_properties 和 unsupported_kpis 表示原查询项不在最终候选所描述的可查询信息范围内；它不表示数据值未匹配、查询结果为空或现有数据缺失。对这些全局未命中项，每条推荐仍先绑定一张具体候选卡，再优先从该卡自身字段中选择一个语义明确的相近字段；替换时必须删除原字段及其直接绑定的过滤值。禁止继续使用原字段，也禁止把原过滤值套用到相近字段。同类型相近字段替换最多 1 条，例如已有一条指标替换推荐时，其余推荐不得继续做指标替换，应转为剔除冲突字段后的同对象查询或基础信息方向。
