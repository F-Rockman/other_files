...
2. 每条推荐必须绑定一个具体 candidate_capability 作为完整证据来源；推荐中的设备类型、子部件关系、特殊对象、属性或指标、查询能力类型必须同时由这一候选支持。禁止把多张候选卡的字段并集当作通用白名单，也禁止跨候选拼接对象和字段。
...
10. 不使用【】插槽、长枚举、“某设备”“某指标”等模糊表达。
11. 推荐问题可以使用列表、数量、字段聚合、趋势和 TopN 等现有查询形式，但不得生成同比、环比、较上期、较同期、去年同期、上月同期等跨周期对比，也不得生成由此派生的增长率、变化率或增减幅问题。即使原始 question 明确包含这类对比表达，推荐中也必须移除，改用候选范围内的趋势、聚合、TopN 或其他可执行方向。
...
14. 设备、关系、取值或结果为空等非属性/指标场景，可使用"暂未匹配到对应对象""暂未识别到可用关联""当前条件暂未匹配到合适业务取值""当前条件下暂未查询到相关数据"等用户可理解表达。
15. 禁止使用“设备不存在”“字段不存在”“对象没有该属性/指标”“元数据”“字段映射”“不支持查询该字段”“暂不支持该查询”及内部技术表达。"""
...
输出 1 到 3 条推荐即可。候选不足、质量低或容易生成无关推荐时，允许少于 3 条；禁止为了凑满 3 条生成无关对象、候选外能力、重复问题或仅换词问题。输出前逐条检查：候选边界、对象关系、无效值、明确结果形态、与原问题差异、推荐之间差异；explain 是否包含当前提问、当前原因和下一步方向（恢复场景）或当前提问和下一步方向（普通场景）；是否保留真实对象和有效条件，是否仅使用用户可理解的业务表达。
"""
...
删除“展示趋势”“趋势”“趋势图”“查看趋势”等查询形态词不算有效简化；查指标时无论是否有时间范围，有无展示趋势视为同一指标查询，不能只靠省略、删除、补充或改写趋势表达生成推荐。推荐问题与原问题只差趋势表达时视为语义一致，禁止输出；三条推荐之间也不得只靠趋势表达差异区分。TopN 是查询形态，不算有效查询条件；删除 TopN、Top5、排名最高等表达不算有效简化。不得只把 KPI 做轻微泛化，例如“总带宽”改成“带宽”但保留所有核心条件。

禁止指标替换：原问题查询设备A或子部件A的指标A时，不得推荐查询同对象的指标B，也不得用相近指标、同类指标或其他性能指标补足三条。可以删除指标条件进入退化路径，但不能替换指标。

同类型简化最多生成 1 条推荐。简化类型包括删除时间、删除定位条件、删除过滤条件、删除聚合、删除对象条件和范围调整；例如已有一条推荐通过删除时间简化，其余推荐不得继续删除时间来凑数。不足 3 条时，按本场景退化路径补足，而不是重复同类简化。
