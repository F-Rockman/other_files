...
- link_query 推荐优先参考候选 examples 的表达结构；examples 仍不是当前环境事实。
...
7. 不虚构设备、IP、MAC、指标、属性值、过滤值、告警名、端口名等事实；具体枚举值仅可来自相关元数据。
8. 优先业务相关、可回答、原对象一致、填写成本低、表达自然的问题。
9. 不生成诊断、异常分析、预测、处置或配置问题；不暴露 SQL、表结构、字段名、数据库、模型、规则、候选或评分。
10. 不使用【】插槽、长枚举、“某设备”“某指标”等模糊表达。
11. 推荐可用列表、数量、字段聚合、趋势、TopN；不得生成同比、环比、较上期、较同期、去年同期、上月同期等跨周期对比，也不得生成增长率、变化率或增减幅。即使原始 question 明确包含这类对比表达，推荐中也必须移除，改用候选内趋势、聚合、TopN 或其他方向。
12. 拒答恢复场景（recovery_strategy 非空，或存在 refusal_message/refusal_detail）禁止继承或生成未来时间。不得推荐明天、后天、下周、下月、明年、未来某天等相对未来时间，也不得推荐明确晚于当前日期的绝对时间；若原问题包含未来时间，优先删除时间条件，或仅继承上下文中明确非未来的时间范围。
...
17. 设备、关系和取值之外的非属性/指标场景，可使用"暂未匹配到对应对象""暂未识别到可用关联"等用户可理解表达；不为查询后无数据维护单独原因模板。
18. 禁止使用“设备不存在”“字段不存在”“对象没有该属性/指标”“元数据”“字段映射”“不支持查询该字段”“暂不支持该查询”及内部技术表达。"""
