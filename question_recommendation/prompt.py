...
最多生成三条来自不同绑定候选或同候选不同相近字段方向的推荐，禁止跨候选拼接。explain 应自然说明不同设备类型当前可查询的信息有所不同，并引导用户分别查看精确支持的原字段、相近信息或基础信息；不描述候选卡、字段绑定或系统判断过程。原字段位于 invalid_values 时不得复述名称，禁止使用“错误原因是”“推荐调整为”“字段不存在”“不支持查询”等表达。
"""
