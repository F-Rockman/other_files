...
explain 直接帮助用户理解下一步，不写成错误分析报告或推荐系统处理日志：

1. 默认使用一句连贯自然的话，只有确实需要表达先后步骤时才使用两句。直接说明推荐问题能帮助用户从哪些方向继续确认，不以拒答原因或系统结论开头。
2. 不复述“当前查询涉及多个业务域”“建议选择具体设备类型”等拒答内容，不完整复述原问题，不解释系统为什么选择这三条推荐；内容必须与 recommends 实际方向一致。
3. 禁止使用“错误原因是”“失败原因是”“推荐调整为”“建议调整为”“推荐方向是”“基于上述原因”“针对该错误”“系统建议”“支持查看”“可查看”等报告式、能力说明式或流程式表达。
4. 优先自然连接“不同对象的信息有所不同”和下一步查询方向，可使用“可以先……”“可以分别……”“先确认……后再……”，但不要每次机械使用同一句式。
5. 有恢复要求时，只说明用户能够理解的当前阻碍，不复述 invalid_values，不暴露恢复策略、规则或内部判断。
6. explain 中提及的设备和子部件名称必须逐字沿用 recommends 中实际使用的名称，禁止用父类或泛化名称替换，例如不得把“闪存存储”改写为“存储设备”。唯一明确设备类型仍须逐字使用原始 devices[].device_type；有子部件时保留父子关系。多个设备类型时不归因于单一类型；没有明确设备类型时不虚构对象。
7. 使用“暂未匹配到对应对象/相关属性内容/合适业务取值”“暂未采集到相关指标数据”“暂未识别到可用关联”“当前条件下暂未查询到相关数据”等委婉表达。
8. 禁止使用“设备不存在”“字段不存在”“对象没有该属性/指标”“不支持查询该字段/指标”“暂不支持该查询”及内部技术表达。

自然表达示例：

- “不同设备类型记录的状态信息有所不同，可以先分别查看服务器健康状态、网络设备通信状态和闪存存储连接状态。”
...
最多生成三条来自不同绑定候选或同候选不同相近字段方向的推荐，禁止跨候选拼接。explain 应自然说明不同设备类型当前可查询的信息有所不同，并引导用户分别查看精确支持的原字段、相近信息或基础信息；当前可查询信息中暂未匹配到原查询项时，先查看相近信息，没有相近信息时再查看基础信息。不描述候选卡、字段绑定或系统判断过程，也不暴露“全局未命中”等内部结论。原字段位于 invalid_values 时不得复述名称，禁止使用“错误原因是”“推荐调整为”“字段不存在”“不支持查询”等表达。
"""
