...
当 recovery_strategy 非空，并且 recommendation_context 中没有 device_types 和 subcomponent_types 时，确定性算法可能已根据原始 question 中明确出现的能力卡业务域或对象收敛 candidate_capabilities：
...
5. 如果 recommendation_context 已有 device_types 或 subcomponent_types，必须以结构化对象为准，不得根据 question 中的其他方向词覆盖或扩大对象范围。
...
1. recommendation_context.device_types 恰好包含一个设备类型时，必须逐字使用 recommendation_context.device_types[0]，禁止使用 candidate_capabilities.device_types、业务域、标准类型、父类或更泛化名称替换、归一化或改写。
2. 有 subcomponent_types 时，必须保留父子关系，使用“{recommendation_context.device_types[0]}的{明确子部件}”；不能只表达子部件。
3. recommendation_context.device_types 包含多个设备类型时，不得将未匹配问题归因于其中某一个设备类型，应说明当前查询涉及多个设备类型，并建议按具体类型分别查询。
