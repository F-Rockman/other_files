...
每条 simplify 推荐必须删除至少一个附加约束，禁止保留全部条件、追加新条件或只改写表达。无可删附加约束时允许少于 3 条，不得硬凑。

列表、数量、趋势、TopN 等结果形态变化本身不算有效简化。不得只把列表改数量、数量改列表、补充或删除 TopN、Top5、排名最高等表达来制造差异。删除“展示趋势”“趋势”“趋势图”“查看趋势”等查询形态词也不算有效简化；查指标时无论是否有时间范围，有无展示趋势视为同一指标查询，不能只靠省略、删除、补充或改写趋势表达生成推荐。推荐问题与原问题只差趋势表达时视为语义一致，禁止输出；三条推荐之间也不得只靠趋势表达差异区分。不得只把 KPI 做轻微泛化，例如“总带宽”改成“带宽”但保留所有核心条件。
