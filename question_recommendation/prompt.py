...
当 refusal_message 或 refusal_detail 明确点名具体缺失属性时，即使它不在 invalid_values 中，也禁止从 question、properties、subcomponent_types 或 examples 继承，且不得变形成子部件、列表、数量或统计。例如“缺少属性1字段”时禁止推荐属性1信息、属性1列表和属性1数量。泛化的“字段检索失败/缺少字段/未找到匹配字段”不得据此删除内容。继续保留其他有效条件；没有安全替代项时回退到候选内不依赖该属性的方向。拒答原因中的“缺少字段”等内容仅用于判断，写 explain 时必须转译为用户可理解的业务表达，禁止复制或近义复述内部原因。
...
15. explain 禁止出现“元数据”“字段”“列”“表”“映射”“SQL”“数据库”“模型”“规则”等内部实现词，也禁止使用“无法直接回答”“无法回答”“不能回答”等失败宣告。内部原因只能用于判断，必须转译为“当前可查询的设备类型A信息中暂未包含属性1相关内容”等面向用户的业务表达。
16. 禁止使用"设备不存在""字段不存在""对象没有该属性/指标""不支持查询该字段/指标""暂不支持该查询"及内部技术表达。

自然表达示例：

- 正确："你希望查询IP地址A对应的设备类型A的属性1，但当前可查询的设备类型A信息中暂未匹配到属性1相关内容。可以先查看该设备类型A的属性2、属性3或指标1。"
- 错误："属性1在现有数据中暂未匹配到相关内容。"
- 错误："元数据中缺少属性1相关字段，无法直接回答。"
- 正确："你希望查看设备类型A的属性1，但不同设备类型记录的相关信息并不完全一致。可以分别按推荐中的具体设备类型继续查询。"
- 正确："这个查询包含的条件较多，可以先减少时间或范围条件，确认结果后再逐步补充。"
"""
...
必须输出正好 3 条推荐。输出前逐条检查：候选边界、对象关系、无效值、明确结果形态、与原问题差异、三条间差异、explain 一致性。若 explain 包含内部实现词或失败宣告，必须改写为用户业务表达后再输出。
"""
...
- candidate_field_analysis 存在全局未命中项时，对象归属直接复用全局 explain 第 9 条：必须将未匹配内容归属到 recommends 中实际使用的具体设备类型或父子对象。本条只约束原因语义，应表达为“当前可查询的信息范围暂未包含该查询项”；禁止使用“现有数据中暂未匹配到”“当前数据中没有”“没有查到”或“查询结果为空”等数据检索结果表达。
