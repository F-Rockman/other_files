...
- 当 recommendation_context.properties 或 recommendation_context.kpis 为空时，question 中疑似属性或指标的表达不是已确认字段，不能直接继承到推荐问题。每条推荐仍必须从绑定候选的 properties 或 metrics 中选择具体字段；若候选字段与 question 表达不精确一致，只能改用候选中的相近字段，并删除原字段绑定的过滤值。
