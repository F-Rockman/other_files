...
3. 只有候选 locators 支持的设备定位类型才可继承；不支持的定位条件不得进入推荐问题，但仍应保留其有效 device_type 作为对象方向。
4. 尽量继承仍有效的对象、定位条件、属性、指标、时间和范围；禁止继承 invalid_values，也禁止从 question、拒答原因或 examples 中找回它们。
5. 不虚构设备、IP、MAC、指标、属性值、过滤值、告警名、端口名或其他事实。具体枚举值仅可来自相关元数据明确提供的业务含义。
6. 优先业务相关、可回答、原对象一致、填写成本低、表达短而自然的问题。
7. 不生成诊断、异常原因分析、预测、处置或配置操作问题；不暴露 SQL、表结构、字段名、数据库、模型、规则、候选或评分。
8. 不使用【】插槽、长枚举、“某设备”“某指标”等模糊表达。
...
1. 使用一到两句连贯自然的话。先委婉说明当前查询暂时卡在哪里，再直接告诉用户可以先查什么、确认什么或减少什么条件。
2. 不完整复述原问题，不逐项罗列条件，不解释系统为什么选择这三条推荐；内容必须与 recommends 实际方向一致。
3. 禁止使用“错误原因是”“失败原因是”“推荐调整为”“建议调整为”“推荐方向是”“基于上述原因”“针对该错误”“系统建议”等报告式或流程式表达。
4. 优先使用“可以先……”“可以分别……”“先确认……后再……”等面向行动的自然表达，但不要每次机械使用同一句式。
5. 有恢复要求时，只说明用户能够理解的当前阻碍，不复述 invalid_values，不暴露恢复策略、规则或内部判断。
6. 唯一明确设备类型必须逐字使用原始 devices[].device_type；有子部件时保留父子关系。多个设备类型时不归因于单一类型；没有明确设备类型时不虚构对象。
