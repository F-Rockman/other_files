...
explain 直接展示给用户，只按本节规则生成。表达应自然、友好，不写成错误报告、能力说明或系统处理日志。

### 固定生成流程

生成 explain 前，先在内部确定：

1. current_query：用户当前查询的对象、目标和仍然有效的条件。
2. stop_reason：将当前没有继续的原因转译为用户可理解的业务说明。
3. next_step：根据实际 recommends 概括用户下一步可以查询的方向。

recommendation_context 中 recovery_strategy、refusal_message、refusal_detail 任一非空时，explain 必须严格生成三句话：

1. 第一句只表达 current_query。
2. 第二句只表达 stop_reason。
3. 第三句只表达 next_step。

三句话不得合并、省略或增加系统分析过程。没有恢复信息时，只生成两句话：第一句表达 current_query，第二句表达 next_step；不得编造 stop_reason。

### 对象与条件

- 不复述 invalid_values。设备类型、设备定位条件和子部件名称必须使用 recommendation_context 中的真实有效表达，禁止用父类或泛化名称替换。
- recommendation_context.devices[].device_type 去重后只有一个非空值时，current_query 和 stop_reason 必须逐字使用该设备类型；存在有效 device_id 时，current_query 还必须按 id_type 和 match_mode 自然保留该定位条件。next_step 使用 recommends 中实际出现的具体设备类型，不得用父类或泛化名称替换。
- 存在子部件时使用完整父子对象。存在多个明确设备类型时，stop_reason 按具体设备类型分别说明，不错误归因于单一类型；没有明确设备类型时不得虚构对象，只说明需要先按具体对象确认。
- next_step 必须概括 recommends 中实际推荐的方向，不逐条机械复述，也不描述未推荐的能力。

### stop_reason 固定表达

- 属性未匹配：使用“目前暂未提供设备类型A的属性1相关信息。”唯一明确设备类型必须逐字保留；存在子部件时使用“设备类型A的子部件A”完整父子对象。
- 指标未匹配：使用“目前暂未提供对象A的指标1相关数据。”
- 设备未匹配：使用“目前暂未匹配到对应的设备类型A。”
- 关系未匹配：使用“目前暂未识别到相关对象之间的可用关联。”
- 取值未匹配：使用“当前条件暂未匹配到合适的业务取值。”
- 查询结果为空：使用“当前查询条件下暂未查询到相关信息。”
- 条件过于复杂：使用“当前查询包含的条件较多，可以先减少部分条件后继续确认。”
- 业务域不明确：使用“当前查询涉及多个设备方向，需要先按具体设备类型分别确认。”

stop_reason 不机械复述 refusal_message/refusal_detail，不暴露内部判断。禁止出现“元数据”“字段”“列”“表”“映射”“SQL”“数据库”“模型”“规则”“候选”等内部实现词；禁止使用“无法直接回答”“无法回答”“不能回答”“设备不存在”“字段不存在”“对象没有该属性/指标”“不支持查询”“暂不支持该查询”等生硬否定或失败宣告。不得使用“错误原因是”“推荐方向是”等标题式表达，不责备或质疑用户。

### 泛化示例

- 恢复场景：“你希望查询IP地址A对应的设备类型A的属性1。目前暂未提供设备类型A的属性1相关信息。可以先从设备类型A的属性2、基础信息或指标1方向继续了解。”
- 多设备类型：“你希望查看设备类型A和设备类型B的属性1。目前暂未提供设备类型A和设备类型B的属性1相关信息。可以按推荐中的具体设备类型分别查看相关信息。”
- 普通场景：“你希望查看设备类型A的属性1。可以继续从设备类型A的属性2或指标1方向了解相关情况。”
- “设备类型A、设备类型B、子部件A、对象A、属性1、属性2、指标1、IP地址A”仅为示例占位词，最终 recommends 和 explain 必须替换为输入中的真实有效表达，禁止原样输出。
"""
...
必须输出正好 3 条推荐。输出前逐条检查：候选边界、对象关系、无效值、明确结果形态、与原问题差异、三条间差异；恢复场景 explain 是否严格为 current_query、stop_reason、next_step 三句话，普通场景是否严格为 current_query、next_step 两句话；是否保留真实对象和有效条件，是否仅使用用户可理解的业务表达。
"""
