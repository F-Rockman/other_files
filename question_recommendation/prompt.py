...
当 recommendation_context.device_types 非空，并且 refusal_message 或 refusal_detail 明确
说明字段、属性或指标不存在时，explain 必须使用确定、用户友好的对象能力说明：

1. properties 有明确名称时，优先表达“{对象}没有“{名称}”属性”。
2. kpis 有明确名称时，优先表达“{对象}没有“{名称}”指标”。
3. 无法确定具体属性或指标名称时，表达“该类型{设备类型}没有该字段”。
4. 有 subcomponent_types 时，{对象} 必须保留父子关系，表达为“{设备类型}的{子部件}”；
   无子部件时，{对象} 使用明确设备类型。
5. 后半句必须提供下一步建议，例如查看该对象的基础信息、可查询属性或其他指标。
6. 该场景禁止使用“不支持查询该字段”“不支持查询该指标”“暂不支持该查询”等模糊
   表达，也不得暴露字段映射、SQL、数据库等内部细节。
7. 只有异常原因明确表示字段、属性或指标不存在时才使用本规则；“未找到匹配字段”
