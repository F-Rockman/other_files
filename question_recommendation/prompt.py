...
当 refusal_message 或 refusal_detail 明确指出一个具体属性不存在、缺少对应字段或无法提供时，必须在最终表达阶段将该属性视为本次推荐的冲突查询项：

1. 只有失败原因明确点名一个具体属性时才执行剔除。例如“缺少节点字段”明确点名“节点”；“字段检索失败”“缺少字段”“未找到匹配字段”等未明确具体属性的原因不得据此删除原问题内容。
2. 即使该属性没有出现在 recommendation_context.invalid_values 中，也不得从 question、recommendation_context.properties、subcomponent_types、examples 或其他输入中重新继承。
3. 不得把缺失属性改写或变形成子部件、列表、数量、统计、分组或其他查询形态。例如“缺少节点字段”时，禁止推荐“有哪些节点”“节点信息”“节点列表”“节点数量”或“按节点统计”。
4. 本节优先于原问题参数继承、明确列表或数量形态继承、Basic 兜底和三条推荐多样性要求；不得为了保持原形态或凑足差异继续使用缺失属性。
5. 除该缺失属性外，必须继续继承仍有效的设备定位条件、设备类型、时间、子网范围和其他有效条件。
6. 相关 metadata_tables 中存在一个唯一、明确相似的业务描述时，允许按“唯一相似查询项替换”生成一条替换推荐；否则回退到 candidate_capabilities 范围内的基础信息、列表、数量或其他不依赖该属性的方向。
7. explain 应使用“当前可查询的{对象}信息中，暂未匹配到相关属性内容”等委婉表达，不得向用户复述“字段不存在”“缺少字段”等内部原因。
...
“明确缺失属性剔除”场景同样适用本节，但相似替换最多只能恢复一条推荐；其他推荐仍不得继续使用已明确缺失的原属性。
