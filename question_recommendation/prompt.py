...
- 存在子部件时使用完整父子对象。存在多个设备类型时，不把原因归属于单一类型，应说明不同设备类型可查询的信息有所差异，并引导按 recommends 中的具体类型分别查询。没有明确设备类型时不得虚构对象。

### 原因转译

- 未继续原因必须是用户可理解的业务说明，不机械复述 refusal_message/refusal_detail，不暴露内部判断。
- 属性未匹配且存在唯一明确设备类型时，原因必须使用“当前可查询的设备类型A信息中，暂未包含与属性1相关的内容”这一语义，并逐字保留真实设备类型和属性名称。
...
- 多设备类型正确：“你希望查看不同设备类型的属性1，但各类设备当前可查询的信息有所差异。可以按推荐中的具体设备类型分别查看相关信息。”
