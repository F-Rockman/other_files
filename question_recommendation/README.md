...
总数、多少、几个等表达时保持数量或数量统计形态。normal 场景的三条推荐均保持明确
形态，并通过过滤方向、对象范围、业务维度或分组方向体现差异。error 场景中形态仍有效
时继续保留；形态或其必要条件与失败恢复冲突时，恢复策略优先。未明确形态时完全沿用
现有主路由、相邻候选和推荐多样性逻辑。
...
明确列表或数量形态时，error 场景仍优先保留该形态；只有失败原因表明该形态或必要条件
不适合继续使用时，才允许恢复策略调整形态。
...
`description_cn`。元数据辅助候选排序和 LLM 理解当前环境真实业务含义。除未匹配场景
中唯一、明确相似查询项的一条替换推荐外，元数据不能突破设备能力规格。
...
这是候选能力边界的唯一受控例外，不新增相似度算法，也不得暴露物理列名、表名或“字段”
概念。
