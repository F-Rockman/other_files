...
- 调用 LLM 前还会生成内部 `field_inheritance_policy`。当 `context.properties` 或
  `context.kpis` 为空时，分别设置
  `allow_question_property_inheritance=false` 或
  `allow_question_kpi_inheritance=false`，表示原问题中的属性或 KPI 表达不能直接作为
  已确认字段继承；推荐问题仍必须从绑定候选的 `properties` 或 `metrics` 中选择具体字段。
- `candidate_field_analysis` 用于处理“上游已经结构化传入字段，但候选不支持”的场景；
  `field_inheritance_policy` 用于处理“上游没有结构化传入字段，原问题文本里可能含有字段
  词”的场景。两者都只影响 LLM 表达，不改变候选召回、排序或公开上下文结构。
- 全局未命中时，每条推荐优先从当前绑定候选自身字段中选择语义相近项；替换后不得继承
  原字段绑定的过滤值。例如不能将“运行状态正常”直接改成“状态正常”。
- 当字段继承策略不允许从原问题继承字段时，即使原问题包含“运行状态正常”这类表达，
  也不能生成“运行状态正常的网络设备”或“状态正常的网络设备”；可以改为查看绑定候选
  实际支持的状态类信息，或回退到同对象基础信息。
...
Chat user prompt 会追加三类确定性辅助分析：

| 名称 | 来源 | 作用 |
|---|---|---|
| `candidate_field_analysis` | `context.properties/kpis` 与最终候选字段精确比较 | 标记所有候选均不支持的结构化原属性或 KPI |
| `simplify_analysis` | `recovery_strategy == "simplify"` 时从上下文提取 | 告诉 LLM 只能删除哪些结构化附加约束 |
| `field_inheritance_policy` | `context.properties/kpis` 是否为空 | 控制原问题文本中的属性或 KPI 表达能否作为已确认字段继承 |
...
| `CAPABILITY-FIELD-INHERITANCE` | 无实时元数据 | 上下文没有结构化属性或 KPI | `test_field_inheritance_policy_reflects_structured_fields`、`test_chat_prompt_blocks_question_field_when_context_has_no_structured_property` |
...
- 字段继承必须先满足 `field_inheritance_policy`。当
  `allow_question_property_inheritance=false` 时，原问题中的属性表达不是可继承原属性，
  不能触发“原属性精确命中白名单”规则；KPI 同理。
