...
确定性召回不调用 LLM 或 Embedding。最终 LLM 输出先校验 JSON 结构，再删除仍包含
`candidate_field_analysis` 已确认 unsupported 字段原文的推荐项；除此之外不补足或改写。
改造只作用于推荐模块，不参与后续 SQL 或问数执行流程。
...
能力卡匹配显式别名。例如 FATAP 命中网络设备能力，AP/无线接入点命中 FITAP，PON设备同时
...
`simplify` 优先于空 `intention` Basic。推荐器会额外生成内部
`simplify_analysis.removable_constraints`，只把结构化上下文中确定可删除的约束交给
LLM，例如子网范围、时间、有效设备定位值、告警条件、`avg/min/max/sum` 聚合，以及多
KPI 中第二个及之后的 KPI。设备类型、子部件、`count/count_distinct`、`top_n`、单个
KPI、属性和“列表/数量/趋势”等结果形态不会进入这个清单。

- 默认保留原业务对象、父子关系和至少一个 KPI、属性、告警或关系等核心查询目标。
- 每条推荐必须删除 `removable_constraints` 中至少一项；不在清单里的内容不能被解释成
  复杂条件或失败原因。
- 可以删除时间、子网、设备定位值、告警条件、可删除聚合，以及多 KPI 中的额外 KPI。
- 三条推荐优先分别删除不同的单一条件；不足三种时，再组合删除多个条件。
- 不能保留全部条件，也不能追加新条件；原问题没有可删除约束时，允许少于 3 条，避免
  硬凑无关推荐。
- 列表、数量、有哪些、以列表形式展示、趋势、展示趋势、TopN 等只是结果形态，不算有效
  简化；不能把这些内容当成复杂修饰。
...
再拆分同一 KPI 的不同聚合，并按原问题顺序选择，输出 1 到 3 条，质量不足时不硬凑，
不能把已拆开的意图重新组合。
...
- 没有加载到元数据，或所有 `column_description` 均为空时，相关候选的 `properties`
  ...
  原问题重新继承。
- 调用 LLM 前会分析原查询属性和 KPI 的候选支持情况。上游已结构化传入字段时，优先
  与最终候选字段做忽略大小写的精确比较；上游未传字段且无可用实时元数据时，再用全量
  能力卡字段归属索引从原问题做保守最长命中。
- `candidate_field_analysis` 统一输出全局未命中项：同名字段存在多个来源时，只要任意
  一个来源与当前查询层级兼容，就不标记为未支持；只有全部来源不兼容时才进入
  `unsupported_properties` 或 `unsupported_kpis`。
- 字段归属裁决只消费能力卡里的属性、指标事实：设备级字段只能服务设备级查询，子部件
  字段只有在原问题明确点名该子部件，或字段名自身包含该子部件类型/别名时，才服务
  子部件查询。
- 全局未命中时，每条推荐优先从当前绑定候选自身字段中选择语义相近项；替换后不得继承
  原字段绑定的过滤值。例如不能将“运行状态正常”直接改成“状态正常”。
- 当原问题未点名子部件时，子部件上的“运行状态”等字段不能证明设备支持该属性；可以
  改为查看绑定候选实际支持的设备级信息，或回退到同对象基础信息。
...
所有 Prompt 文本集中维护在 `question_recommendation/prompt.py` 中。`recommender.py` 会按
结构化上下文调用 `_build_system_prompt(...)`，在核心规则和输出规则之间插入当前请求需要的
场景片段。修改 Prompt 时应优先检查已有规则是否能覆盖，无法覆盖时再补充或调整对应片段。

Chat user prompt 会追加确定性辅助分析：

| 名称 | 来源 | 作用 |
|---|---|---|
| `candidate_field_analysis` | 结构化字段优先；缺失时用全量能力卡字段归属索引补充抽取 | 标记当前查询层级不支持的原属性或 KPI |
| `simplify_analysis` | `recovery_strategy == "simplify"` 时从上下文提取 | 告诉 LLM 只能删除哪些结构化附加约束 |
...
Prompt 的高影响规则由契约测试保护。修改或压缩 Prompt 时，应同步保留对应语义和测试；
测试名称用于定位契约，不要求 Prompt 保留固定长文案。

| 规则范围 | 加载条件 | 契约测试 |
|---|---|---|
| 核心候选边界、对象边界、字段来源、结果形态、去重、未来时间、多意图和 explain 口径 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules`、`test_core_prompt_requires_actionable_natural_explain` |
| 无恢复要求弱推荐路径 | `intention` 非空且 `recovery_strategy` 为空 | `test_normal_runtime_prompt_loads_only_normal_fragment` |
| simplify 降复杂度 | `recovery_strategy == "simplify"` | `test_simplify_fragment_overrides_empty_intention_basic` |
| Basic 与空 intention Basic | 空 intention 或 `recovery_strategy == "basic"` | `test_empty_intention_uses_basic_fragment_for_other_strategies`、`test_nonempty_intention_selects_only_matching_recovery_fragment` |
| 拒答业务方向 | 有恢复策略且没有结构化设备或子部件 | `test_recovery_direction_fragment_requires_recovery_and_no_structured_object` |
| 子网范围 | 存在有效 `subnet` | `test_subnet_fragment_is_selected_only_by_structured_subnet` |
| 实时元数据字段来源 | 存在非空 `column_description` | `test_metadata_fragment_requires_nonempty_column_description` |
| 无实时元数据字段白名单与全局未命中 | 没有非空 `column_description` | `test_no_metadata_fragment_uses_candidate_fields_as_strict_whitelist`、`test_candidate_field_analysis_marks_field_missing_from_all_final_candidates` |
| 动态片段选择不依赖文本关键词 | 始终 | `test_question_and_refusal_text_do_not_select_dynamic_fragments` |

关键细则：

- 只有候选 `locators` 支持的定位类型可以继承；不支持的定位值不得进入推荐问题。
- 原问题没有明确列表或数量时，只在同对象、同候选边界、同等可回答性下按列表优先。
- 任意场景都不能只靠列表/数量、趋势有无或 TopN 有无制造差异；`simplify` 只有删除真实
  `removable_constraints` 后才允许跨列表/数量形态。
...
- 文本补召回会先判断字段归属：没有明确子部件锚点时，只使用设备级属性或指标触发设备
  候选；子部件属性或指标不得反向证明设备支持同名字段。
...
Prompt 要求生成 1 到 3 条语义不同的问题；候选不足或质量低时不强行凑满。推荐器会删除
仍包含 unsupported 字段原文的推荐项，不对剩余推荐做补齐、截断或改写；无法解析时返回
`{"recommends": [], "explain": ""}`。
