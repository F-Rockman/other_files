...
所有 Prompt 文本集中维护在 `question_recommendation/prompt.yaml` 中。每个片段都使用
统一结构：

```yaml
core_rules:
  description: 固定核心规则，始终加载
  prompt: |
    Prompt 内容
```

`recommender.py` 负责读取 YAML、校验每个片段包含 `description` 和 `prompt`，并按结构化
...
名称用于定位契约，不要求 Prompt 保留固定长文案。Prompt 中使用规则 ID 标记核心契约，
便于后续压缩时逐条审计。

| 规则 ID | 所属片段 | 加载条件 | 契约测试 |
|---|---|---|---|
| `CORE-CAPABILITY-BOUNDARY` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-DEVICE-BOUNDARY` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-SPECIAL-OBJECT` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-DEVICE-CONDITION` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-FIELD-SOURCE` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-VALIDITY` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-SAFETY` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-RESULT-FORM` | 核心 | 始终，由 LLM 理解原问题形态 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-DEDUP` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-ALLOWED-FORMS` | 核心 | 始终 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
| `CORE-FUTURE-TIME` | 核心 | 拒答恢复时 | `test_core_prompt_keeps_global_and_text_interpretation_rules` |
...
| `NORMAL-WEAK-PATH` | 无恢复要求 | `intention` 非空且 `recovery_strategy` 为空 | `test_normal_runtime_prompt_loads_only_normal_fragment` |
