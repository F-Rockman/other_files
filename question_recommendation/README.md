...
当异常原因明确说明字段、属性或指标不存在，并且已识别明确设备类型时，Prompt 要求
`explain` 直接说明该设备或其子部件没有对应属性或指标，并给出下一步建议；禁止使用
“不支持查询该字段/指标”等模糊表达。“未找到匹配字段”等名称匹配失败说明不会被推断为
设备类型缺少字段。该确定性文案仅在上游恰好识别出一个设备类型时使用，并且必须逐字
保留 `recommendation_context.device_types[0]`，不得改写为候选能力中的标准或泛化类型。
