...
- 没有加载到元数据，或所有 `column_description` 均为空时，相关候选的 `properties`
  ...
  原问题重新继承。
- 原字段未命中能力卡时，LLM 可以从同对象字段白名单中选择最多三个语义相近字段；替换
  后不得继承原字段绑定的过滤值。例如不能将“运行状态正常”直接改成“状态正常”。
- 相近字段不足三条时，回退到不依赖具体字段的同对象信息查询，移除冲突字段及其关联
  取值，并继续继承其他有效对象、父子关系、定位条件、时间和子网范围。
...
- 无实时元数据时，能力卡字段是最终白名单。优先使用同对象最多三个相近字段，替换字段
  不继承原过滤值；不足三条时补充同对象信息查询，不切换为数量或其他对象。
...
`explain` 不写成错误分析报告或推荐系统处理日志，而是直接帮助用户理解下一步。使用
一到两句连贯自然的话，委婉说明当前查询暂时卡在哪里，并直接告诉用户可以先查什么、
确认什么或减少什么条件。无需完整复述原问题，也不解释系统为什么选择这些推荐。

禁止使用“错误原因是”“失败原因是”“推荐调整为”“建议调整为”“推荐方向是”
“基于上述原因”“针对该错误”“系统建议”等报告式或流程式表达。优先使用“可以先……”
“可以分别……”“先确认……后再……”等行动表达，但不要机械复用同一句式。
